    ...
    <div class="mb-6 flex items-center justify-between">
        <div class="flex items-center space-x-6">
            ...
            <p>
                <strong>
                  <RouterLink :to="{name: 'profile', params:{'id': post.created_by.id}}">
                    {{ post.created_by.name + ' ' + post.created_by.surname}}
                  </RouterLink>
                </strong>
            </p>
        </div>
    ...
    <div class="my-6 flex justify-between">
        <div class="flex space-x-6">
            <div class="flex items-center space-x-2">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6" @click="likePost(post.id)">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z"></path>
                </svg>

                <span class="text-gray-600 text-sm">{{ post.likes_count }} нравится</span>
            </div>
            ...
            <div class="flex items-center space-x-2">
                ...
                <RouterLink :to="{name: 'postview', params: {id: post.id}}" class="text-gray-600 text-sm">{{ post.comments_count }} комментариев</RouterLink>
            </div>
        </div>
